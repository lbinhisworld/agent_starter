...
1. 约定技术栈与模块划分（`frontend` / `frontend-vue` / `backend`）。
...
- 若以 `docs/agents/architect/00-core.md` 初始化为 **architect owner**，默认聚焦**架构判断、拆工派工、审查与验收口径**；未获明确授权前，不直接修改 `frontend/`、`frontend-vue/`、`backend/` 业务代码。
...
| `docs/knowledge/product/` | 产品专题知识卡（与 business-product 分层配合，长篇方法论外移） |
...
| `frontend/` | 主业务前端 UI 与本地/代理 AI 调用封装 |
| `frontend-vue/` | Vue 登录/管理端、首页影子页（`home.html` / `src/home/`）与 `src/api/client.ts` 统一鉴权与续期头 |
