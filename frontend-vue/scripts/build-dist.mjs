...
// 本工程根 = frontend-vue/（唯一自包含工程）
const projectDir = path.resolve(__dirname, '..');
// vite 产物目录（npm run build 生成），作为 build-dist 的源
const distDir = path.join(projectDir, 'dist');
// build-dist 最终发布产物目录
const releaseDir = path.join(projectDir, 'release');
// 本地 require（解析本工程 node_modules，用于 babel/lightningcss/terser）
...
const homeLegacyBundleSources = [
  'static/js/utils.js',
  'static/js/api.js',
  'static/js/storage-http-adapter.js',
  'static/js/storage-indexeddb-adapter.js',
  'static/js/task1BusinessInsight.js',
  'static/js/preliminaryRequirement.js',
  'static/js/storage.js',
  'static/js/core/problem-follow-shared.js',
];

const wrappedModuleSources = new Set([
  'static/js/core/problem-case-api.js',
]);

const copiedFiles = [
  'static/config.js',
  'static/config.local.js',
  'static/styles.css',
  'static/js/config.js',
  'static/js/auth-runtime.js',
  'vite-home-guard.js',
  'login.html',
  'admin.html',
  'model-config.html',
...
const copiedDirs = ['vue-auth-assets', 'static'];
const minifiedHtmlFiles = ['home.html', 'login.html', 'admin.html', 'model-config.html'];
const minifiedCssFiles = ['static/styles.css'];
const minifiedJsFiles = ['static/js/home-legacy.bundle.js'];
...
const requiredDistFiles = [
  ...
  'static/js/config.js',
  'static/js/auth-runtime.js',
  'static/js/home-legacy.bundle.js',
...
function relToDist(relPath) {
  return path.join(releaseDir, relPath);
}
...
function loadMinifyToolchain() {
  ...
  try {
    ...
    const generatorModule = localRequire('@babel/generator');
    const lightningcss = localRequire('lightningcss');
...
function loadObfuscationToolchain() {
  ...
  try {
    const terser = localRequire('terser');
...
function transformModuleSource(relPath, source) {
  ...
  if (relPath === 'static/js/core/problem-case-api.js') {
...
function buildBundle(relPaths, bundleName) {
  ...
  return [
    `/* ${bundleName} */`,
    '/* Generated by frontend-vue/scripts/build-dist.mjs */',
    '',
...
function buildHomeHtml() {
  ...
  [
    ...
    '  <script src="./static/js/preliminaryRequirement.js"></script>',
    '  <script src="./static/js/storage.js"></script>',
    '  <script src="./static/js/core/problem-follow-shared.js"></script>',
  ].forEach((line) => {
  ...
  html = insertAfterLine(
    ...
    '  <script src="./static/js/home-legacy.bundle.js"></script>',
  );
...
async function buildDist() {
  // 产物目录 = release/（源是 dist/，由 vite build 生成）
  ...
  writeDistFile('static/js/home-legacy.bundle.js', buildBundle(homeLegacyBundleSources, 'home-legacy.bundle.js'));
...
function printSummary() {
  ...
  function walk(currentDir) {
    ...
    for (const entry of entries) {
      ...
      files.push(path.relative(releaseDir, absPath).replace(/\\/g, '/'));
    }
  }

  walk(releaseDir);

  process.stdout.write('[build-dist] generated frontend-vue/release\n');
