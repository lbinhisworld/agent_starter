...
> **已下线（2026-04-13）**：主站 `index.html` / Vue `home.html` 顶栏「工具知识库」入口、`#toolsView` 与相关存储键已从产品移除；下文保留为历史设计记录，**不再描述当前行为**。

> 最近更新：2026-03-19（下线前与实现一致）。
...
- 视觉风格：
  - 使用垂直时间线设计：左侧线 + 蓝色圆点，参考「问题详情 → 沟通历史 → 任务过程日志」样式（问题详情侧滑过程日志的标签字号等以 `styles.css` 中 `.problem-detail-history-panel` 为准，见 `数字化问题跟进阶段设计.md` §0.6）。
