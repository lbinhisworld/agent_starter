...
> 最近更新：2026-04-01。与 main.js、js/task6PainPoint.js、js/storage.js、js/communication-history.js 实现一致。  
> 变更（2026-04-01）：**LLM 输入**——不再向 user 注入整段 `requirement_logic`；改为 **需求逻辑「需求背后的深层动机」**（`parseRequirementLogicFromMarkdown` → `deep_motivation`）+ 初步需求 **`painPointRadar` 各条 `itGap`**（`buildResolvedPreliminaryRequirement`）；`generatePainPointAnnotation(valueStream, item, caseKey)` / `generatePainPointForOneStep(..., item, caseKey)`。  
> 变更（2026-03-31）：**修改链二段确认**——`painPointModificationRegenerateNotifyBlock` 确认后下发 `painPointSessionsBlock` 并播放刷新动画完毕，**不再**自动调用 `runPainPointAnnotationForNextStep`；首环须由用户在 Session 计划卡片上点击「自动顺序执行」再开始（与主流程首次进入 task6 一致）。  
> 变更（2026-03-31）：**Session 计划块**仅保留「自动顺序执行」按钮，已移除「手动顺序执行」入口；`runPainPointAnnotationForNextStep` 仍供自动顺序内部、LLM 重试及历史聊天中单步「确认」链式下一环使用。  
> 变更（2026-03-19）：**自动顺序执行**改为每步不自动确认、仅在工作区绘制痛点块，全部结束后下发「是否确认所有痛点标注结果？」与「确认所有」按钮；点击「确认所有」后统一标记所有环节为已确认并弹出任务完成确认。**工作区实时渲染**依赖 main 暴露 `setCurrentProblemDetailItem`，单步/自动顺序每步 LLM 返回后调用以同步当前项，使 `renderProblemDetailContent()` 能重绘出最新痛点块。  
> 变更（2026-03-21）：**存储契约**——在 `MODE=local`（IndexedDB）或 `MODE=online`（HTTP 适配器）下，`updateDigitalProblemPainPointSessions` / `updateDigitalProblemPainPointStep` 由 `storage.js` 统一通过 `adapter.updateDigitalProblem` 写入问题单缓存，与 `getDigitalProblems()` 一致；`runPainPointAnnotationAutoSequential` 循环内用 `currentItem.createdAt` 刷新当前项。若痛点 session 仅写入 `localStorage` 而不更新适配器缓存，会导致 `painPointSessions` 进度误判、自动顺序仅执行一环即弹出「确认所有」。单步 `stepIndex` 与 session 上 `stepIndex` 比较使用 `Number(...)`，避免类型不一致导致单步未写入。  
> 注：沟通历史侧滑中 `task6LlmQueryBlock` 等时间线条目的标签字号由 `styles.css` `.problem-detail-history-panel` 统一控制，见 [数字化问题跟进阶段设计.md](../数字化问题跟进阶段设计.md) §0.6。

**模块职责**

- **整图一次标注**：基于「价值流图」与「深层动机 + 初步需求 itGap」（见上），调用大模型为所有环节一次性标注 `painPoint`，解析 JSON 后通过 `mergePainPointIntoValueStream` 合并进原始价值流图，更新存储并推进工作流阶段（0、1、2），推送完成态卡片并触发下一任务通知。
- **按环节单步标注**：根据 `painPointSessions` 列表逐环节调用大模型，推送 `painPointStepCard`（展示该环节大模型返回的痛点信息，下方含确认、重做、修正、讨论四个按钮）。**每次 session 的大模型调用均向时间线推送标签为「LLM-查询」的内容卡片**（`task6LlmQueryBlock`），标签右侧标注**环节名称**，内部包含输入/输出子卡片，承载大模型输入（fullPrompt）与输出（llmOutputRaw）的完整数据，设计借鉴 task1～5 的 LLM-查询 块。Session 计划卡片（`painPointSessionsBlock`）仅提供「**自动顺序执行**」：用户点击后由 `runPainPointAnnotationAutoSequential` 连续跑完各环节；每步大模型返回后**不自动确认**，仅在工作区价值流图对应环节节点中实时绘制该环节的痛点块（红底白字、边缘红色闪烁），全部结束后下发「是否确认所有痛点标注结果？」与「确认所有」；用户点击「确认所有」后，将所有环节的聊天区内容块（`painPointStepCard`、`task6LlmQueryBlock`）及该条「确认所有」块（`painPointAllDoneConfirmBlock`）设为已确认，时间线中对应 LLM-查询 块在标签右侧显示「确认」标签，并弹出任务完成确认。`runPainPointAnnotationForNextStep` 仍用于自动顺序内部逐环、以及历史会话中单步卡「确认」后链式下一环（兼容旧数据）。
...
#### `async generatePainPointAnnotation(valueStream, item, caseKey)`
 ...
 * @param {Object} item - 当前案例详情项（`requirementLogic` 解析 `deep_motivation`；`buildResolvedPreliminaryRequirement` 取 `painPointRadar` 含 itGap）。
 * @param {string|number} caseKey - 提示词覆盖键。
...
- **行为**：`buildPainPointAnnotationUserContext(item)` 组装「需求背后的深层动机」与 `painPointRadar` 的 itGap JSON，与 `value_stream` 一并写入 user；以 `PAIN_POINT_ANNOTATION_PROMPT` 为 system 调用 `fetchDeepSeekChat`；返回 `content`、`usage`、`model`、`durationMs`。由 `runPainPointAnnotation` 内解析 JSON 并合并。

---

#### `async generatePainPointForOneStep(stepName, stageName, valueStream, item, caseKey)`
 ...
 * @param {Object|string} valueStream - 价值流图。
 * @param {Object} item - 当前案例详情项（同整图，用于深层动机与 itGap）。
 * @param {string|number} caseKey - 提示词覆盖键。
...
- **行为**：与整图共用同一 user 上下文块（深层动机 + painPointRadar/itGap + 全量 value_stream），再强调目标环节；拼装 `fullPrompt`（system + 分隔 + userContent），以 `PAIN_POINT_ANNOTATION_PROMPT` 为 system 调用 `fetchDeepSeekChat`；将返回的 `content` trim 后与 `fullPrompt` 一并返回。由 `runPainPointAnnotationForNextStep` 使用并写入 `task6LlmQueryBlock.llmInputPrompt`。
...
- **行为**：禁用「自动顺序」按钮后进入循环：调用 `runPainPointAnnotationForNextStep`（每步会更新存储、推送 `task6LlmQueryBlock` 与 `painPointStepCard`、调用 `setCurrentProblemDetailItem(updated)` 并执行 `renderProblemDetailContent()`，工作区实时显示新痛点块）；**不在循环内调用 `applyPainPointStepConfirm`**。当 `runPainPointAnnotationForNextStep` 返回 `null`（无下一未标注环节）时：推送 `painPointAllDoneConfirmBlock`（文案「是否确认所有痛点标注结果？」、按钮「确认所有」），重绘聊天区与过程日志，然后退出循环。循环结束后根据是否还有未完成 session 恢复按钮的 disabled 状态。用户点击「确认所有」由 main 委托处理：将所有 `painPointStepCard`、`task6LlmQueryBlock`、`painPointAllDoneConfirmBlock` 标为 `confirmed: true`，保存并重绘后调用 `showTaskCompletionConfirm('task6', '痛点标注')`。
...
- **状态与渲染**：`el`、`currentProblemDetailItem`、**`setCurrentProblemDetailItem`**（main 挂到 window 的 setter，用于在更新存储后同步 main 内部的 `currentProblemDetailItem`，使 `renderProblemDetailContent()` 能读到最新 valueStream，实现自动执行过程中工作区痛点块实时渲染）、`renderProblemDetailContent`、`buildLlmMetaHtml`、`renderProblemDetailChatFromStorage`、**`getProblemDetailChatMessages`**（main 提供的 getter，返回当前 `problemDetailChatMessages` 数组；因 main 中该变量为 let 未挂 window，模块重绘聊天区时需通过此 getter 获取，避免传入空数组导致聊天内容被清空）
...
- **存储**：`updateDigitalProblemValueStreamPainPoint`、`updateDigitalProblemPainPointStep`、`updateDigitalProblemPainPointSessions`、`getDigitalProblems`（启用 HTTP/IndexedDB 适配器时，痛点 session/单步更新与列表读取同源，见文首「存储契约」）
...
- **按钮事件**：Session 计划上「自动顺序」→ `runPainPointAnnotationAutoSequential(currentProblemDetailItem)`。「确认」痛点单步 → `applyPainPointStepConfirm(...)`；若全部完成则弹出任务完成确认；若未全部完成则**自动触发下一 session 环节的大模型调用**（刷新 `currentProblemDetailItem` 后调用 `runPainPointAnnotationForNextStep`），供历史会话曾分步确认的场景兼容。**「确认所有」**（自动顺序全部跑完后出现）：main 在聊天区渲染 `painPointAllDoneConfirmBlock`（文案「是否确认所有痛点标注结果？」、按钮「确认所有」）；用户点击后，将当前聊天消息中所有 `painPointStepCard`、`task6LlmQueryBlock` 及该条 `painPointAllDoneConfirmBlock` 设为 `confirmed: true`，保存并重绘聊天区与过程日志，再调用 `showTaskCompletionConfirm('task6', '痛点标注')`。
- **任务完成确认**：用户点击「已完成」后，与 task5 类似，推进状态并切换到下一任务（如 task7）。
- **时间线 LLM-查询**：沟通历史（`js/communication-history.js`）对 `task6LlmQueryBlock` 的展示与 task1～5 一致：标签「LLM-查询」、右侧备注为**环节名称**（`stepName`）、输入/输出双子卡片及 token/耗时（来自 `llmMeta`）；当 `parsed.confirmed === true` 时在标签右侧显示「确认」标签。用户点击痛点单步卡片的「确认」时，main 将对应 `painPointStepCard` 及最近一条 `task6LlmQueryBlock` 标记为已确认；用户点击「确认所有」时，所有 `task6LlmQueryBlock` 及 `painPointAllDoneConfirmBlock` 均标为已确认，时间线中对应 LLM-查询 均显示「确认」标签。
- **工作区实时渲染**：单步或自动顺序执行时，每次大模型返回后模块调用 `updateDigitalProblemPainPointStep` 写回 valueStream 与 painPointSessions，并从 `getDigitalProblems()` 取最新项，除赋值 `global.currentProblemDetailItem` 外**必须**调用 `setCurrentProblemDetailItem(updated)`，使 main 内部的 `currentProblemDetailItem` 与存储一致，随后 `renderProblemDetailContent()` 才能用最新 valueStream 重绘工作区，新产生的痛点内容块（红底白字、边缘红色闪烁）才会实时显示；否则工作区会沿用旧项数据不刷新。
