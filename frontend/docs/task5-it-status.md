...
> 最近更新：2026-04-01。与 main.js、js/communication-history.js、js/task5ItStatus.js 实现一致。沟通历史侧滑中 `task5LlmQueryBlock` 等时间线条目的标签字号由 `styles.css` `.problem-detail-history-panel` 统一控制，见 [数字化问题跟进阶段设计.md](../数字化问题跟进阶段设计.md) §0.6。  
> **主流程**：Session 计划 + 逐环节「自动顺序执行」；单环节 user 中 **`value_stream` 仅为当前目标环节的设计摘录**（去 `itStatus`/`itPlan` 等标注字段），与 **完工确认「请修改」→ 新版提示词** 侧 **`buildTask5ItStatusSingleStepPromptSnapshot`** 对齐，避免整图与过程日志撑爆上下文（详见阶段设计 §5.8.1 文首变更）。
...
### Session 主流程与单环节上下文（与修改链对齐）

- **计划下发**：任务启动确认后由 `setupItStatusSessionPlanAfterTaskStart` 推送 `taskContextBlock`（价值流 / 需求逻辑 / IT 现状 json）、`itStatusSessionsBlock`，并在存储中写入 `itStatusSessions`（与 `generateItStatusSessions` 顺序一致的全局环节序号）。
- **逐环节执行**：`runItStatusAnnotationForNextStep` / `runItStatusAnnotationAutoSequential` 调用 `generateItStatusForOneStep(..., stepIndex)`；user 消息中 **「仅含当前目标环节的价值流设计摘录」** 由 `buildValueStreamDesignSliceForStep(valueStream, stepIndex)` 生成（单阶段、单 step，已剥离 IT 标注字段），**不再**把整图 `value_stream` 塞进单步提示词。
- **整图兼容**：`runItStatusAnnotation`（整图一次）仍保留，用于历史/兼容；新主路径以 Session + 单步为准。
- **修改链 — 新版提示词首轮**：`main.js` 的 `requestRefinementFromFeedback` 在 task5 且能解析 `task5LlmQueryBlock.stepIndex` 时，用 **`buildTask5ItStatusSingleStepPromptSnapshot(item, stepIndex)`** 重建与单步执行一致的 **`fullPrompt`**，作为「当前大模型调用提示词」参与修订；**该轮不加载过程日志**（全任务统一策略，见阶段设计 §5.8.1）。
- **提示词覆盖**：按案例 `getProblemDetailChatStorageKey` 读写 `getTask5ItStatusPromptOverride` / `setTask5ItStatusPromptOverride`；修改链二次确认后可 `setTask5ModAwaitingAutoStrip` 以便自动执行前回滚价值流上 IT 标注（`rollbackValueStreamItStatus`）。
