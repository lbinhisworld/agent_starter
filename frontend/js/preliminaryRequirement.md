...
> 最近更新：2026-03-30。与 `main.js` 中首页「解析」及问题详情「初步需求」展示流程保持一致；历史详情 `RQ-001`、标题提炼（≤30 字）与绿色标题样式见 [数字化问题跟进阶段设计.md](../数字化问题跟进阶段设计.md) 文首 **FE-20260330-23** 与本文 §3.10、§4。首页解析写入的 `task1LlmQueryBlock` 在沟通历史中的展示样式见 `styles.css` `.problem-detail-history-panel` 与 [数字化问题跟进阶段设计.md](../数字化问题跟进阶段设计.md) §0.6。
...
**作用**：生成「历史详情」Tab 内容 HTML：按时间线展示历次提交的需求详情，每项为可折叠块（`.preliminary-history-item`），标题行内 **左侧可点击主区**（`.preliminary-history-item-header-main`）展示 **需求编号**（`RQ-001` 样式，与顺序一致）、**标题提炼**（最多 30 字，绿色 `.preliminary-history-item-title`，优先用条目已存 `title`，否则从正文首行提炼）及时间；右侧 **删除** 钮（`.preliminary-history-item-delete-btn`）经 `main.js` `deletePreliminaryHistoryItemAt` 写回 `requirementDetailHistory` 或清空虚拟 `requirementDetail` 后 `renderProblemDetailContent`；展开后显示该次提交的原始需求全文（`.preliminary-history-item-body`）。

**入参**：

- `item: Object` — 当前问题项；若含 `requirementDetailHistory`（`Array<{ timestamp, content, title? }>`）则按该数组渲染；否则用 `requirementDetail` + `createdAt` 生成一条。
- `escapeHtml: function(string): string` — 转义 HTML 的函数。

**返回**：历史时间线 HTML 字符串，插入 `.problem-detail-card-body-preliminary-history`。展开/收起与删除均由 `main.js` 的 `setupPreliminaryHistoryItemToggle(container)` 在卡片上做事件委托（展开区为 `.preliminary-history-item-header-main`；`hidden` 切换显示）。

**相关**：`formatRequirementDetailHistoryCode(serial)` → `RQ-001`；`extractRequirementDetailHistoryTitle(text)` 首行提炼 ≤30 字；`buildRequirementDetailHistoryEntry(content)` 供写入历史时统一带 `title`。
...
- **历史详情**：按时间线展示历次提交的需求详情；每条为可折叠块，标题行左侧为 **RQ-001** 样式编号、中间为标题提炼（≤30 字，绿色 `.preliminary-history-item-title`）、右侧为时间（由 `formatPreliminaryHistoryTime` 格式化），点击标题行展开/收起，展开后显示该次提交的原始需求全文。内容由 `buildPreliminaryHistoryHtml(item, escapeHtml)` 生成；展开/收起由 `main.js` 的 `setupPreliminaryHistoryItemToggle(container)` 在「初步需求」卡片上做事件委托，通过 `body.removeAttribute('hidden')` / `body.setAttribute('hidden', '')` 及父节点 class `preliminary-history-item-expanded` 控制，样式见 `styles.css`（`.preliminary-history-item-body[hidden] { display: none !important; }` 等）。

**数据**：首次「启动跟进」与每次深度提炼/补充合并写入时，由 `buildRequirementDetailHistoryEntry(content)` 生成 `requirementDetailHistory` 元素：`{ timestamp, content, title }`（`title` 为提炼标题，最多 30 字）。旧数据仅含 `timestamp/content` 时，渲染阶段仍用正文首行补全展示标题。存储与 HTTP 适配器（`storage.js`、`storage-http-adapter.js`）对 `requirementDetailHistory` 做持久化与前后端字段映射。
