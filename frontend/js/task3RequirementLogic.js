/**
 * [INPUT]: 依赖 `fetchDeepSeekChat`（api）、全局挂载。
 * [OUTPUT]: `parseRequirementLogicFromMarkdown`、`extractCausalChainNodesFromLogicSummary`（逻辑链条总结→因果节点）、`generateRequirementLogicFromInputs`、`buildTask3LlmQueryMessage` 等。
 * [POS]: Task3 需求逻辑构建与需求理解工作区「需求逻辑」卡数据源。
 *
...
注意：每个 ## 标题下方必须紧跟具体分析内容，不可留空。`;
  ...
  /**
   * 从「逻辑链条总结」正文中提炼因果链节点（用于需求理解工作区子卡片 + 箭头可视化）。
   * 顺序：箭头分隔（→/➜/⇒/->/=>）→ 全角【】括弧片段 → 逗号/分号后「所以/因此/进而…」→ 整段单节点。
   * @param {string} text - logic_summary 段落纯文本
   * @returns {string[]} 非空节点文案数组
   */
  function extractCausalChainNodesFromLogicSummary(text) {
    const raw = String(text || '').trim();
    if (!raw) return [];
    const stripNoise = (s) =>
      String(s || '')
        .replace(/^\s*(?:因为|由于|鉴于)\s*/u, '')
        .replace(/\s+$/u, '')
        .trim();

    const arrowSplit = raw
      .split(/\s*(?:→|➜|⇒|->|=>)\s*/u)
      .map((s) => stripNoise(s))
      .filter((s) => s.length > 0);
    ...
    const brRe = /【([^】]+)】/g;
    let bm;
    while ((bm = brRe.exec(raw)) !== null) {
      const inner = (bm[1] || '').trim();
      if (inner) bracketChunks.push(inner);
    }
    if (bracketChunks.length >= 2) return bracketChunks;

    const soSplit = raw.split(/(?:，|；)\s*(?:所以|因此|进而|导致|最终实现|从而)\s*/u);
    if (soSplit.length >= 2) {
      const parts = soSplit.map((s) => stripNoise(s)).filter((s) => s.length > 0);
...
\`\`\``;
  ...
  global.extractCausalChainNodesFromLogicSummary = extractCausalChainNodesFromLogicSummary;
