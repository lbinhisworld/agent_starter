...
(function (global) {
  ...
  /**
   * 切换应用主视图显隐状态。
   * @param {'home'|'detail'|'problemDetail'|'taskTracking'|string} view - 目标视图。
   * @returns {void}
   */
  ...
  function toggleProblemDetailHistory(open) {
    ...
    // 关闭前将焦点移出面板，再设 aria-hidden；否则关闭按钮仍持焦会导致「aria-hidden 祖先内存在焦点」的 a11y 告警
    if (!isOpen) {
      const ae = typeof document !== 'undefined' ? document.activeElement : null;
      if (ae && typeof panel.contains === 'function' && panel.contains(ae) && typeof ae.blur === 'function') {
        ae.blur();
      }
      const opener = e?.btnProblemDetailHistory;
