...
> 最近更新：2026-03-24（FE-20260324-09：修改链路 task11 双阶段确认、`coreBusinessObjectSystemPromptOverride`、`skipContextBlocks`）。与 main.js、js/communication-history.js 中 task11 流程及过程日志展示一致。沟通历史侧滑内过程日志标签的统一字号见 `styles.css` `.problem-detail-history-panel` 与 [数字化问题跟进阶段设计.md](../数字化问题跟进阶段设计.md) §0.6、§5.8.1。
...
在 `index.html` 中，`coreBusinessObject.js` 置于 `js/rolePermission.js` 之后、`main.js` 之前，确保上述依赖已加载。本模块通过 `global.executeXxx = …` 挂到 `window`/`globalThis` 上；**main.js 中调用 `executeCoreBusinessObjectTaskOnConfirm` 等须使用 `globalThis.xxx`（或 `window.xxx`）**，勿依赖裸全局名（部分环境下仅挂属性不会产生裸标识符解析）。
...
### 4.1 `generateCoreBusinessObjectForStepWithStrictPrompt(stepName, stageName, stepIndex, valueStreamJson, globalItGapJson, localItGapJson, rolePermissionJson, objectPoolFromPreviousStep, roleRegistryJson)`

**作用**：使用严格提示词（V3.3 架构闭环与冷启动版：全局静态底座 + 存量承接判定 + 冷启动基础实体初始化 + 极简实体约束）针对单环节调用大模型，返回该环节的 JSON。**当前「自动顺序执行」「手工逐项确认」均调用此函数**。**可选第 8 参 `systemPromptOverride`（FE-20260324-09）**：非空时整段替换默认 V3.3 system 正文（来自案例字段 `coreBusinessObjectSystemPromptOverride`，用于 task11 修改链路）。

**入参**：`valueStreamJson`、`globalItGapJson`、`localItGapJson`（全量 `localItGapByStep` 序列化）；**`rolePermissionJson`** 为 `buildCoreBusinessObjectContextJson` 产出的 **按环节对齐** 数组序列化（每项含 `stepIndex`、`rolePermissionAnalysis`，供 `pickCurrentStepJson` 取出「当前环节角色与权限分析」）；**`roleRegistryJson`** 为工作区「角色清单」同源数据（`role_name`、`linked_steps`、可选 `functional_description`），写入 User Prompt 的【角色清单 json】，**不得**再使用全量推演 JSON 充当角色清单；`objectPoolFromPreviousStep` 为上一环节业务对象池。`localItGapJson` 在函数内仍按当前 `stepIndex` 用 `pickCurrentStepJson` 取当前环节有效载荷（含 `analysis` 字段时取 `analysis`）。

**System Prompt 约束**（以 `generateCoreBusinessObjectForStepWithStrictPrompt` 内实际字符串为准）：角色为严谨首席系统架构师，强调“如无必要，勿增实体”；核心准则保留 `Project_ID` 与全局枚举；建模指令新增“冷启动与基础实体初始化判定”（Initialization Check），并保留资产独立性判定、存量承接判定与状态驱动；输出为严格 JSON，根级含 `当前环节`、`架构审计结论`、`核心骨架清单`、`存量对象承接`。`核心骨架清单` 项含 `对象中文名`、`对象ID`、`对象类型`（基础产出/业务主产出/过程记录）、`独立存在理由`、`溯源锚点`、`嵌套子表模块`（模块对象数组）、`角色权责`、`本环节终态`。解析层兼容 `嵌套子模块` 与旧键 `嵌套子表模块`。

**User Prompt**：注入端到端压缩、**全局 ITGap 分析压缩版 JSON**、角色清单、本环节名称与**当前环节角色汇总设计压缩 JSON**（优先取压缩块，缺失回退分析 JSON）及角色权限切片；**不再**注入上一环节对象池（与骨架阶段一致）。`parseCoreBusinessObjectModel` / `mapSkeletonBoToEntity` 将 `核心骨架清单` 映射为内部 `entities` 供工作区 view。
...
**输出结构（单 JSON 对象）**：根级包含 `当前环节`、`架构审计结论`、`核心骨架清单`。`核心骨架清单` 每项包含 `对象中文名`、`对象类型`（主产出/过程记录/状态更新）、`独立存在理由`、`溯源锚点`、`设计理由`、`嵌套子表模块`、`角色权责`、`关联关系`、`本环节终态`。
...
- **User**：拼接「项目 + 环节」说明、端到端事务流 JSON、可选的全局/局部 ITGap JSON，并明确要求「直接输出该环节的 JSON 对象，不要 markdown 代码块或说明文字」。
...
主流程将 `buildCoreBusinessObjectNodeCardsHtml(parsedModel)` 的 HTML 插入工作区（例如在 `itStrategyPlanViewingSubstep === 1` 时），并依赖 main.js 中已有的折叠、tab 切换等事件绑定（若选择与 role-permission 相同的 class 命名规范，可复用部分逻辑）。**核心业务对象工作区**：用户确认启动后，在角色清单下方展示 **「全局静态底座 (Static Anchor)」** 卡片（`buildStaticAnchorCardHtml`，main.js），样式与角色清单一致（绿底白字、可折叠），内容为聚合 `globalItGapCompressed`、`e2eFlowCompressed`、`roleRegistry` 的总 JSON，view/json 双 Tab。**IT 策略工作区价值流环节卡片**（与角色与权限同列）：某环节已写入 `coreBusinessObjectJson` 时，标题栏在角色列表后显示 **「生成业务对象N个」**（`getCoreBusinessObjectEntityCountForWorkspaceStep`，与 entities 条数一致）。
...
**返回**：`{ valueStream, globalItGap, localItGapByStep, rolePermissionByStep, roleRegistryJson, roleRegistryContext }`。`rolePermissionByStep` 优先由 `rolePermissionSessions` 按 `stepIndex` 组装（`rolePermissionAnalysis`）；无 session 时回退 `parseRolePermissionModel(getLatestConfirmedRolePermissionContent(...))` 并按数组下标对齐。`roleRegistryJson` / `roleRegistryContext` 来自 `getRolePermissionRegistryForDisplay(item)`（与工作区「角色清单」一致）。若 `valueStream` 无效则返回 `null`。

**与单环节 LLM 入参**：`localItGapByStep` 在此为**全量**环节数据（供任务确认时推送到过程日志的「角色汇总设计压缩 json」上下文块）；`generateCoreBusinessObjectForStepWithStrictPrompt` 在拼装 User Prompt 时会**内部**按当前 `stepIndex` 仅保留 **±1 相邻环节** 与当前项，见 §4.1。

**main.js 与 session 数据源**：聊天区渲染 `coreBusinessObjectSessionsBlock` 时若问题记录中无 `coreBusinessObjectSessions`，会回退使用消息内的 `sessions`（故仍显示「待推演」）；自动/手工执行环节 LLM 时须使用 `resolveCoreBusinessObjectSessionsForRun`：优先读问题记录，缺失则从最近一条 `coreBusinessObjectSessionsBlock` 回填并 `updateDigitalProblemCoreBusinessObjectSessions`，避免 `sessions === []` 被误判为「已全部推演完毕」。

### 8.2 `executeCoreBusinessObjectTaskOnConfirm(item, valueStream, callbacks, options)`
...
**options（可选）**：`{ skipContextBlocks: true }` 时**不**推送下列四条上下文块（FE-20260324-09：task11 修改链路二次确认后仅重新下发 `coreBusinessObjectSessionsBlock`）。

**过程日志推送**（默认，`skipContextBlocks` 未开启）：依次推送 **4 条** `coreBusinessObjectContextBlock`（仅入沟通历史过程日志，不在聊天区展示），每条带 `contextLabel` 与对应 `contextJson`，过程日志中标签为「上下文」、标签右侧显示备注：

| 顺序 | contextLabel | contextJson |
|------|--------------|-------------|
| 1 | 价值流设计 json | valueStream（`buildCoreBusinessObjectContextJson` 产出） |
| 2 | 全局 ITGap 分析压缩版 json | globalItGap |
| 3 | 角色汇总设计压缩版 json | localItGapByStep |
...
| `generateCoreBusinessObjectForStepWithStrictPrompt(..., rolePermissionJson, systemPromptOverride?)` | 单环节推演（严格提示词，当前主入口），返回该环节的 LLM 文本 |
...
| `executeCoreBusinessObjectTaskOnConfirm(item, valueStream, callbacks, options?)` | 执行任务确认后的推送与 session 生成逻辑，返回结果或错误信息 |
...
价值流 (valueStream)
    ...
    → LLM 返回 content + llmInputPrompt（【系统】/【用户】拼接）
    → 写入 session.coreBusinessObjectJson；pushAndSaveProblemDetailChat(task11LlmQueryBlock，可含 `skeletonObjectCount`) 再 pushAndSave(coreBusinessObjectAnalysisCard)（过程日志大模型调用仅展示前者，与 task9 一致；时间线标题栏显示「+N 对象」见 `communication-history.js`）
...
- **样式**：所有 class 均带 `problem-detail-core-business-object-*` 或 `problem-detail-card-core-business-*` 前缀，便于在 `styles.css` 中单独维护或与角色权限卡片风格统一。关键样式：局部 ITGap 与核心业务对象设计标题栏（明黄底 #ffeb3b、深蓝字 #1a237e）；对象类型分组栏、对象卡片标题（明黄字 #ffeb3b）；对象卡片内子卡片标题与角色与权限「过去操作」一致（#5dc9b4、0.78rem）；字段定义表淡灰线、数据类型列与设计意图列；树形引导线（`.problem-detail-core-business-object-design-list` + `.problem-detail-core-business-object-design-item::before`）。**标题栏对象名**：「核心业务对象设计」与各 `object_role` 分组标题右侧展示 **名称1｜名称2｜…**（`.problem-detail-core-business-object-names-head`），不再使用「N个对象」个数标注。
