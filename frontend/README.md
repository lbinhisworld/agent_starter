...
- **ITGap 分析**：端到端事务流构建 → 全局 ITGap 分析 → 角色汇总设计（逐环节）。
...
- **大模型（DeepSeek）**  
  将 `config.example.js` 复制为 `config.local.js`，在 `config.local.js` 中填入 `DEEPSEEK_API_KEY`（及可选 `DEEPSEEK_API_URL`、`DEEPSEEK_MODEL`）。`config.local.js` 已加入 .gitignore，不会提交到 Git。用于解析数字化问题、生成 BMC、需求逻辑、价值流、IT 现状/痛点标注、全局/角色汇总设计及聊天区意图提炼与回复。
- **企业分析 API**（画布模式）  
...
| `styles.css` | 样式与 BMC 九宫格、价值流、工作区卡片、沟通历史（含 `.problem-detail-history-panel` 下 `--history-label-fs` 等统一标签字号）任务标题栏颜色（已完成=绿色、进行中=蓝色）等布局 |
...
| `js/storage.js` | 本地存储封装：`MODE=local` 时委托 `STORAGE_INDEXEDDB_ADAPTER`，`MODE=online` 时委托 `STORAGE_HTTP_ADAPTER`；数字化问题、聊天、痛点 session/单步等经 `updateDigitalProblem` 与缓存一致（见 `js/storage-indexeddb-adapter.md`、`docs/task6-pain-point.md`） |
| `js/communication-history.js` | 沟通历史提取与按任务分段：inferTaskIdFromMessage、shouldIncludeInCommunicationHistory、getCommunicationsByTask(createdAt, chats)、getCommunicationsAsTimeline（不写入存储，仅从聊天记录聚合）；面板 HTML 内汇总区 SVG 尺寸与 `styles.css` 中图标样式一致 |
...
加载顺序需保证依赖前置：`config.js` → `config.local.js` → `js/config.js` → `js/utils.js` → `js/api.js` → `js/storage-http-adapter.js`（online）→ `js/storage-indexeddb-adapter.js`（local 持久化）→ … → `js/storage.js` → … → `main.js`（完整顺序见 `index.html`）。
...
| `docs/task4ValueStream.md` | 任务4 价值流：Mirror→按 VSM 阶段分步加固、V2 初步需求注入、L1/L2 JSON 与 `normalizeTask4LlmToWorkspace`、Session 与存储、工作区展示 |
| `docs/task6-pain-point.md` | 任务6 痛点标注（整图/单步/自动顺序、与存储适配器一致的 session 契约） |
| `js/storage-indexeddb-adapter.md` | local 模式 IndexedDB 适配器：迁移、`storageIndexedDbReady`、与 `storage.js` 的关系 |
| `docs/homepage-design.md` | 首页视图与客户档案列表（含首屏与 IndexedDB 就绪重绘） |
...
| `js/localItGap.md` | 角色汇总设计模块（task9）实现说明 |
...
- 任务6 痛点标注：[docs/task6-pain-point.md](./docs/task6-pain-point.md)；local 存储：[js/storage-indexeddb-adapter.md](./js/storage-indexeddb-adapter.md)
- 首页设计：[docs/homepage-design.md](./docs/homepage-design.md)
