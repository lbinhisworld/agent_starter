...
- **硬门禁**：未出现明确授权词前，owner 默认只写 `docs/agents/**`、`docs/knowledge/**`、相关 `AGENTS.md` 与规则文档；不得改 `frontend` / `frontend-vue` / `backend` 业务代码。
...
- **错误复盘落点（冻结）**：具体“错误现象/根因/处理方法/验证证据”一律写入 `03-active-tasks.md`（或知识卡）；`00-core.md` 仅保留规则与模板，不承载单次事件流水。
